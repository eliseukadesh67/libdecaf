 ...
 * Ported from the historical decaf p521 implementation.  gf_mulw was renamed
 * to gf_mulw_unsigned to match the current per-field interface.
 */

#include "f_field.h"

/* The value*value 64x64->128 multiply is exposed under different names by the
 * per-architecture intrinsics: arch_ref64 provides widemul(uint64_t,uint64_t),
 * while arch_x86_64's widemul takes pointers and its value form is widemul_rr.
 * Normalise to WIDEMUL so this backend is correct on both. */
#if defined(__x86_64__)
#define WIDEMUL(x,y) widemul_rr((uint64_t)(x),(uint64_t)(y))
#else
#define WIDEMUL(x,y) widemul((uint64_t)(x),(uint64_t)(y))
#endif
